/* Diogo Pinto | 103259 |  Projeto 2 IAED | 2021/22 | PARTE 1*/
...
struct Aeroporto aeroportos[N_AERO], temp; /*Vetor de Aeroportos e struct temporária*/
...
int adiciona_voo(){
    ...
    Reservas **reservas = malloc(sizeof(Reservas*));
    if (reservas == NULL){
        printf("No memory.\n");
        exit(0);
    }
    ...
    voos[count_voos].passageiros = 0; 
    voos[count_voos].reservas = reservas;
    voos[count_voos].count_reservas = 0;
